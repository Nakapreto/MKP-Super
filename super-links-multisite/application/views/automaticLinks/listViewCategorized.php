...
<div class="border mt-2">
    ...
    foreach($linksByGroup as $linkGroupSplit){
        if(!$linkGroupSplit->idGroup) {
            $searchInGroup[] = strtoupper($linkGroupSplit->title);
        }
    }
    ...
    if($existLinkWithoutCategory) {
        $actions = '  
                              <a class="btn btn-outline-warning btn-sm spl-actions-view" href="admin.php?page=super_links_automatic_list_view&idCategory=none" data-container="body" data-toggle="popover" data-placement="top" data-content="Ver links dessa categoria"><i class="fas fa-eye"></i></a>';
        $group = [
            0,
            $searchInGroup,
            'Links inteligentes sem categoria',
            $actions
        ];
    ...
    foreach ($groups as $group) {
        ...
        $actions = '  
                              <a class="btn btn-outline-warning btn-sm spl-actions-view" href="admin.php?page=super_links_automatic_list_view&idCategory=' . $group['id'] . '" data-container="body" data-toggle="popover" data-placement="top" data-content="Ver links dessa categoria"><i class="fas fa-eye"></i></a>
                              <a class="btn btn-outline-primary btn-sm spl-actions-edit" href="admin.php?page=super_links_edit_AutomaticGroup&id=' . $group['id'] . '" data-container="body" data-toggle="popover" data-placement="top" data-content="Editar a categoria"><i class="fas fa-pen"></i></a>
    ...
    jQuery(document).ready(function () {
        ...
        function deleteCategory(idGroup){
            return new Promise((resolve, reject) => {
                ...
                <?php $url = SUPER_LINKS_TEMPLATE_URL . '/deleteAutomaticGroup'; ?>
