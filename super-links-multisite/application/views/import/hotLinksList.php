    ...
    <div class="container">
        <div class="py-1">
            <div class="row justify-content-end">
                <div class="col-8">
                    <h3><?= TranslateHelper::getTranslate('Lista de links do Hot Links Ninja') ?></h3>
        ...
        <div class="row">
            ...
            if($links) {
                $projects = isset($this->pageData['projects'])? $this->pageData['projects'] : [];

                foreach ($links as $link) {
                    $link = get_object_vars($link);

                    $redirectSelected = $hotLinksModel->getRedirectTypeSuperLinks($link['redirecionamento']);
                    $optionsRedirect = $addLinkModel->getOptionsRedirectImport($redirectSelected);
                    ...
                    $selectRedirect = '<select class="form-control" id="select_' . $link['id_link'] . '" name="select_' . $link['id_link'] . '"> ' . $options . ' </select>';

                    $urls = ($link['url_afiliado2'] || $link['url_afiliado3'])? '<strong>A:</strong> ' : '';
                    $urls .= $link['url_afiliado'];
                    $urls .= $link['url_afiliado2']? '<br><strong>B:</strong> ' . $link['url_afiliado2'] : '';
                    $urls .= $link['url_afiliado3']? '<br><strong>C:</strong> ' . $link['url_afiliado3'] : '';
                    ...
                    ';
                        ...
                        $palavraChave = strtolower($palavraChave);

                        $linkName = '
                        <button type="button" class="btn btn-sm btn-primary">
                          ' . ucfirst($palavraChave) . '
                        </button>
                    ';
                    }
                    ...
                    $optionsGroup = "";
                    $existProjectHotLinks = false;

                    if(isset($projects[$link['id_projeto']]) && $projects[$link['id_projeto']] != ''){
                        $optionsGroup .= "<optgroup label='Categoria do HotLinks'>";
                        $optionsGroup .= "<option value='" . $projects[$link['id_projeto']] . "' selected>" . $projects[$link['id_projeto']] . "</option>";
                        $optionsGroup .= "</optgroup>";
                        $existProjectHotLinks = true;
                    }

                    $optionsGroup .= "<optgroup label='Categorias do Super Links' class='appendOptionsSPL'>";
                    foreach ($optionsValues as $value) {
                        $optionsGroup .= ($value['selected'] && !$existProjectHotLinks) ? "<option value='" . $value['text'] . "' selected>" . $value['text'] . "</option>" : "<option value='" . $value['text'] . "' >" . $value['text'] . "</option>";
                    }
                    $optionsGroup .= "</optgroup>";

                    $selectGroup = '<select class="form-control groupSuperLinks" id="project_' . $link['id_link'] . '" name="project_' . $link['id_link'] . '"> ' . $optionsGroup . ' </select>';
            ...
            if($links) {
                ...
                <form id="importForm" action="" method="POST">
                    ...
                    <input type="hidden" name="<?= $importModel->getModelName() ?>[pluginToImport]" value="hotLinksPlus">
                </form>
