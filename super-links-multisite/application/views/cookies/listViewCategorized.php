...
<div class="border mt-2">
    ...
    $rowTitles = [
        $groupCookieModel->attributeLabels()['groupName'],
    ...
    if($existLinkWithoutCategory) {
        $actions = '  
                              <a class="btn btn-outline-warning btn-sm spl-actions-view" href="admin.php?page=super_links_cookiePost_view&idCategory=none" data-container="body" data-toggle="popover" data-placement="top" data-content="Ver configurações dessa categoria"><i class="fas fa-eye"></i></a>';
        $group = [
            0,
            'Configurações sem categoria',
            $actions
        ];

        TableHelper::loadRowsCategories($group);
    }
    ...
    foreach ($groups as $group) {
        ...
        $actions = '  
                              <a class="btn btn-outline-warning btn-sm spl-actions-view" href="admin.php?page=super_links_cookiePost_view&idCategory=' . $group['id'] . '" data-container="body" data-toggle="popover" data-placement="top" data-content="Ver configurações dessa categoria"><i class="fas fa-eye"></i></a>
                              <a class="btn btn-outline-primary btn-sm spl-actions-edit" href="admin.php?page=super_links_edit_groupCookie&id=' . $group['id'] . '" data-container="body" data-toggle="popover" data-placement="top" data-content="Editar a categoria"><i class="fas fa-pen"></i></a>
    ...
    jQuery(document).ready(function () {
            ...
            if (confirm("Deseja excluir esta categoria?")) {
                deleteCategory(idGroup).then(result => {
                    if(result.status){
                        ...
                        notifier.notify('warning', 'Não foi possível excluir essa categoria. Remova todas as configurações da categoria antes de excluí-la.')
                    }
                }).catch(err => {
                    notifier.notify('warning', 'Não foi possível excluir essa categoria. Remova todas as configurações da categoria antes de excluí-la.')
                })
        ...
        function deleteCategory(idGroup){
            return new Promise((resolve, reject) => {
                ...
                <?php $url = SUPER_LINKS_TEMPLATE_URL . '/deleteGroupCookie'; ?>
