        ...
        if(array_key_exists('search', $options)) {
            ...
            foreach($options['search'] as $key => $value) {
                ...
                if($value['caseInsensitive']) {
                    $searchCaseInsensitive[\strtolower($key)] = $value;
                } else {
        ...
        foreach ($this->windowLengths as $len) {
            ...
            if($searchResult) {
                ...
                if(!$ignoreWordBoundary) {
                    $followingChar = strtolower($this->getChar(1));
                    ...
                    $previousChar = $partLength - $len > 0 ? strtolower($part->code[$partLength-$len-1]) : null;
